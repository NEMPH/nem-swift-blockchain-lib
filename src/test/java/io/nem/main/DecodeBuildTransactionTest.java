package io.nem.main;

import org.junit.Test;
import org.nem.core.crypto.KeyPair;
import org.nem.core.crypto.PrivateKey;
import org.nem.core.crypto.PublicKey;
import org.nem.core.messages.SecureMessage;
import org.nem.core.model.Account;
import org.nem.core.model.Address;
import org.nem.core.utils.HexEncoder;

import io.nem.swift.crypto.SecureMessageSwiftPayloadDecoder;
import io.nem.util.GzipUtils;

/**
 * The Class BuildTransactionTest.
 */
public class DecodeBuildTransactionTest {

	/** The sample swift msg. */
	final String sampleSwiftMsg = "";
	final String simplePayload = "2e66cc91dc415cb8c51ab5ffc8f38724119a417d86ecfcfc2169576ac224ca92f1081ea1541eb0ef1badfc3cd0f8f4a22967f31f1775057b4b690baf07262fb5eb0e9682df212ce774d4956736c16b9c";

	/**
	 * Test transaction hash.
	 */
	@Test
	public void testDecode() {

		final KeyPair senderKeyPairPriv = new KeyPair(
				PublicKey.fromHexString("90951d4f876e3a15b8507532a051857e933a87269bc0da7400d1604bedc93aec"));
		final KeyPair senderKeyPair = new KeyPair(
				PublicKey.fromHexString("d72ebcdf243f9d217f448d937ac9431ba6ee4984b167015abcfc5b22e169ca23"));
		final KeyPair recipientKeyPairPriv = new KeyPair(
				PrivateKey.fromHexString("c9d930757f69584fc414d0b2b54a0c3aa064996f9b13b70d32c89879724153c1"));
		final KeyPair recipientKeyPair = new KeyPair(
				PublicKey.fromHexString("8043f36622be5c91e00d9977c870935c887ff9050ba0a62207db76dba1a87385"));

		// SecureMessage secureMessage = SecureMessage.fromEncodedPayload(new
		// Account(senderKeyPair), new Account(recipientKeyPairPriv),
		// HexEncoder.getBytes(sampleSwiftMsg));

		String decodedPayloadString = "";
		try {
			// byte [] decodedPayload = secureMessage.getDecodedPayload();
			String a = SecureMessageSwiftPayloadDecoder.decodeAndGzipUncompress(senderKeyPair, recipientKeyPairPriv,
					sampleSwiftMsg);
			// System.out.println(cipher.decrypt(decodedPayload).toString());
			System.out.println(a);
		} catch (Exception e) {
			e.printStackTrace();
		}

	}

}
